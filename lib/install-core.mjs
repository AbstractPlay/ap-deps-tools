...
import { AP_PACKAGES, PLACEHOLDER_VERSION } from "./constants.mjs";
 ...
 */
export function hasManagedPlaceholders(pkgJson, packages) {
  for (const pkg of packages) {
    if (pkgJson.dependencies?.[pkg] === PLACEHOLDER_VERSION) {
      return true;
    }
  }
  return false;
...
/**
 * Write ci-deps / dispatch versions into package.json so npm ci can resolve them.
 *
 * @param {object} pkgJson
 * @param {object} versions
 */
export function applyRealVersions(pkgJson, versions) {
  pkgJson.dependencies = pkgJson.dependencies ?? {};

  if (
    versions.packages.includes(AP_PACKAGES.gameslib) &&
    versions.gameslib &&
    AP_PACKAGES.gameslib in pkgJson.dependencies
  ) {
    pkgJson.dependencies[AP_PACKAGES.gameslib] = versions.gameslib;
  }
  if (versions.packages.includes(AP_PACKAGES.renderer) && versions.renderer) {
    if (AP_PACKAGES.renderer in pkgJson.dependencies) {
      pkgJson.dependencies[AP_PACKAGES.renderer] = versions.renderer;
    }
  }
  if (versions.packages.includes(AP_PACKAGES.recranks) && versions.recranks) {
    if (AP_PACKAGES.recranks in pkgJson.dependencies) {
      pkgJson.dependencies[AP_PACKAGES.recranks] = versions.recranks;
    }
  }
}

/**
 * Run npm ci after temporarily writing real AP versions (placeholders are not on the registry).
 *
 * @param {string} root
 * @param {object} versions
 */
export function bootstrapNpmCi(root, versions) {
  const pkgJson = readJson(packageJsonPath(root));
  if (!pkgJson) {
    throw new Error(`Missing ${packageJsonPath(root)}`);
  }

  applyRealVersions(pkgJson, versions);
  writeJson(packageJsonPath(root), pkgJson);
  console.log("Bootstrap: npm ci (package.json AP deps use ci-deps versions temporarily)");
  execSync("npm ci", { cwd: root, stdio: "inherit" });
}

/**
 * Ensure managed AP dependencies use placeholder versions; set renderer override when needed.
 *
 * @param {string} root
 * @param {object} pkgJson
 * @param {object} versions
...
export function syncPackageJson(root, pkgJson, versions) {
  ...
  for (const pkg of versions.packages) {
    ...
    pkgJson.dependencies[pkg] = PLACEHOLDER_VERSION;
  }
  ...
  if (needsRendererOverride) {
    ...
    pkgJson.overrides[AP_PACKAGES.renderer] = PLACEHOLDER_VERSION;
  }
...
export function installPackages(root, versions) {
  ...
  execSync(`npm install --no-save --save-exact ${pkgs.join(" ")}`, {
    cwd: root,
    stdio: "inherit",
  });
}
