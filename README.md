...
- `package.json` lists exact AP versions (synced by `ap-install-deps`)
...
Writes exact versions to `package.json`, updates the lockfile via `npm install --no-save --save-exact`, writes `ci-deps.<stage>.json`, and sets `GITHUB_OUTPUT` when running in GitHub Actions.
